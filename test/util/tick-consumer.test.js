...
describe('TickConsumer', () => {
  ...
  test('should add a processor to the processorSlots', () => {
    const processor = { isAvailable: jest.fn(), isMatch: jest.fn() };
    tickConsumer.addProcessor(processor);
    expect(tickConsumer.processorSlots).toContain(processor);
  });
